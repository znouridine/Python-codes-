...
###Ecart-type
if len(b1_v_set_f)>1 and len(b1_v_reset_f)>1 :
    ...
    m_re_1 = 0.0
    md_re_1 = 0.0
if len(b2_v_set_f) > 1 and len(b2_v_reset_f) > 1:
    ...
    m_re_2= 0.0
    md_re_2 = 0.0
if len(b3_v_set_f) > 1 and len(b3_v_reset_f) > 1:
    ...
    m_re_3 = 0.0
    md_re_3 = 0.0
if len(b4_v_set_f) > 1 and len(b4_v_reset_f) > 1:
...
color = 'tab:red'
ax1.set_ylabel("Conductance in ( *(Cp="+str(f"{(cp):.2f}")+")) S",fontsize="18", color=color)
...
ax2.plot(b1_Pulse_set_f[:], b1_v_set_f[:], 'r.',label='V_s1: Med='+str(f"{(md_s_1):.2f}")+' My='+str(f"{(m_s_1):.2f}")+' sd='+str(f"{(sd_s_1):.2f}")+'', markersize=dot_plot)
ax2.plot(b1_Pulse_reset_f[:], b1_v_reset_f[:], 'r*',label='V_re1: Med='+str(f"{(md_re_1):.2f}")+' My='+str(f"{(m_re_1):.2f}")+' sd='+str(f"{(sd_re_1):.2f}")+'', markersize=dot_plot)
ax2.plot(b2_Pulse_set_f[:], b2_v_set_f[:], 'b.',label='V_s2: Med='+str(f"{(md_s_2):.2f}")+' My='+str(f"{(m_s_2):.2f}")+' sd='+str(f"{(sd_s_2):.2f}")+'', markersize=dot_plot)
ax2.plot(b2_Pulse_reset_f[:], b2_v_reset_f[:], 'b*',label='V_re2: Med='+str(f"{(md_re_2):.2f}")+' My='+str(f"{(m_re_2):.2f}")+' sd='+str(f"{(sd_re_2):.2f}")+'', markersize=dot_plot)
ax2.plot(b3_Pulse_set_f[:], b3_v_set_f[:], 'g.',label='V_s3: Med='+str(f"{(md_s_3):.2f}")+' My='+str(f"{(m_s_3):.2f}")+' sd='+str(f"{(sd_s_3):.2f}")+'', markersize=dot_plot)
ax2.plot(b3_Pulse_reset_f[:], b3_v_reset_f[:], 'g*',label='V_re3: Med='+str(f"{(md_re_3):.2f}")+' My='+str(f"{(m_re_3):.2f}")+' sd='+str(f"{(sd_re_3):.2f}")+'', markersize=dot_plot)
ax2.plot(b4_Pulse_set_f[:], b4_v_set_f[:], 'k.',label='V_s4: Med='+str(f"{(md_s_4):.2f}")+' My='+str(f"{(m_s_4):.2f}")+' sd='+str(f"{(sd_s_4):.2f}")+'', markersize=dot_plot)
ax2.plot(b4_Pulse_reset_f[:], b4_v_reset_f[:], 'k*',label='V_re4: Med='+str(f"{(md_re_4):.2f}")+' My='+str(f"{(m_re_4):.2f}")+' sd='+str(f"{(sd_re_4):.2f}")+'', markersize=dot_plot)
...
plt.figure()
# bande1
plt.hist(b1_R_set_f*cp, rwidth=0.9, color='k', label='SET')
plt.hist(b1_R_reset_f*cp, rwidth=0.9, color='r', label='RESET')
plt.title("Hist data Bande [" + str(round(bande[0][3],2)*cp) + "," + str(round(bande[0][4]*cp,2)) + "]",fontsize="14")
plt.xlabel("Conductance in ( *(Cp="+str(f"{(cp):.2f}")+")) S",fontsize="18")
