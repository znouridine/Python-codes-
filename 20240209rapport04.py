# -*- coding: utf-8 -*-
"""
Created on Thu Feb  8 12:22:01 2024
...
"""
    ...
    for t in range(20):
        plt.plot(X[t],Rp[t][1], 'r.')
        plt.plot(X[t],Rp[t][4], 'go')
        plt.plot(X[t],Rp[t][5], 'b^')
        plt.plot(X[t],r_th_B,'c_')
    plt.title(name,loc='left')
    plt.title(name1,loc='Right')
    plt.xlabel("N° ligne")
    plt.ylabel("Résistance (K ohm)")
    plt.xlim([0,21])
    plt.ylim([0.1,100])
    ...
    plt.text(1,60,"V1Set: "+str(float("%.2f" % l))+"V", fontdict=font)
    plt.text(1,30,"V1ReSet: "+str(float("%.2f" %m))+"V", fontdict=font)
    plt.text(12,60,"R1Set: "+str(float("%.2f" %o))+"KOhm", fontdict=font)
    plt.text(12,30,"RmaxReSet: "+str(float("%.2f" %p))+"KOhm", fontdict=font)
# Taille C
if via == 4:
