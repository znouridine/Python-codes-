# -*- coding: utf-8 -*-
"""
Created on Wed July 10 15:37:32 2024
...
data = np.loadtxt("20240710/" + name + ".NL", skiprows=3, usecols=range(0, 14))
c2 = data[:, 1];
c4 = data[:, 3];
c9 = data[:, 8];
c10 = data[:, 9]
...
ax1.plot(X0[:, 0], X0[:, 1], 'r.', X1[:, 0], X1[:, 1], 'k.', X0[:, 0], Rp[:], 'c-', X0[:, 0], Rth[:], 'b-', markersize=dot_plot)
ax1.plot(X1[:, 0], Rp[:], 'c-', label='Rp=' + str(int(rp)) + 'k',markersize=dot_plot)
ax1.plot(X1[:, 0], Rth[:], 'b-', label='Rth=' + str(int(r_th * 1e-3)) + ' k',markersize=dot_plot)
...
ax2.plot(X0[:, 3], X0[:, 2], 'r.', X1[:, 3], X1[:, 2], 'k.', markersize=dot_plot)
ax2.set_xticks([])
ax2.set_ylim([1, 7])
...
ax3.plot(X0[:, 3], X0[:, 4], 'r.', X1[:, 3], X1[:, 4], 'k.',markersize=dot_plot)
ax3.set_yscale('linear')
ax3.set_yticks([50,100,150,200])
ax3.set_ylim([10,200])
...
fig.tight_layout()
fig.savefig("20240710/pics/"+name+".png", dpi = 1200)
plt.show()