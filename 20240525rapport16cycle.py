# -*- coding: utf-8 -*-
"""
Created on Thu May 23 15:37:32 2024
...
data = np.loadtxt("20240525/" + name + ".NL", skiprows=3, usecols=range(0, 14))
c2 = data[:, 1];
c4 = data[:, 3];
c9 = data[:, 8];
c10 = data[:, 9]
...
color = 'tab:red'
ax1.set_ylabel('Resistance in ohm', color=color)
...
# ax2 = ax1.twinx()
color = 'tab:blue'
ax2.set_ylabel('Pulse voltage in V', color=color)
ax2.plot(X0[:, 3], X0[:, 2], 'r.', X1[:, 3], X1[:, 2], 'k.')
ax2.set_yscale('linear')
ax2.set_xlabel('Number of pulse')
ax2.grid(color='b', linestyle='--', linewidth=0.7, axis='y')
ax2.set_yticks([1, 2, 3, 4, 5, 6, 7, 8])
# ax2.set_xlim([_a,_b])
ax2.tick_params(axis='y', labelcolor=color)
fig.tight_layout()
fig.savefig("figure_one.png", dpi = 2000)
fig.set_dpi(2000)
plt.show()
# plt.yticks([1,1000,10000,100000])
# plt.xlim([68,117])
#fig.savefig("figure_one_dpi500.png", dpi = 500)