...
plt.plot(X1[:, 0], Rp[:], 'c-', label='Gp= ' + str(f"{(rp):.2f}") + ' uS', markersize=dot_plot)  # Plot Cp
plt.plot(X1[:, 0], Rth[:], 'b-', label='Gth=' + str(f"{(r_th/rp):.2f}") + ' Gp', markersize=dot_plot)  # Plot C th
...
gap_log_ce_et_bo = 40  ## % écart entre le centre et le bord
R_on = np.log10(3000)  ## Limite basse fenêtre mémoire
R_off = np.log10(30000) ## Limite haute fenêtre mémoire
...
plt.plot(X1[:, 0], Rp[:], 'c-', label='Rp= ' + str(f"{(rp):.2f}") + ' uS', markersize=dot_plot)  # Plot Cp
plt.plot(X1[:, 0], Rth[:], 'b-', label='Rth=' + str(f"{(r_th/rp):.2f}") + ' Rp', markersize=dot_plot)  # Plot C th
plt.tick_params(axis='y', labelcolor=color, labelsize= labsize)
plt.tick_params(axis='x', labelcolor=color, labelsize= labsize)
plt.yscale('log')
#plt.ylim([0.8, 70])
