# -*- coding: utf-8 -*-
"""
Created on Fri Aug  8 15:06:39 2025
...
color = 'k'
plt.ylabel('Résistance Log(k$\Omega$)', color=color, fontsize=font)
...
plt.figure(2,figsize= (L/2.54,l/2.54))
color = 'k'
plt.ylabel("Résistance Log(k$\Omega$)",fontsize=font, color=color)
...
x4_s = np.sort(b5_R_set_f)
T4_s = len(x4_s)
y4_s = np.arange(start=1, stop=T4_s + 1, step=1) / float(T4_s)
x4_re = np.sort(b5_R_reset_f)
T4_re = len(x4_re)
y4_re = np.arange(start=1, stop=T4_re + 1, step=1) / float(T4_re)
x5_s = np.sort(b6_R_set_f)
T5_s = len(x5_s)
y5_s = np.arange(start=1, stop=T5_s + 1, step=1) / float(T5_s)
x5_re = np.sort(b6_R_reset_f)
T5_re = len(x5_re)
y5_re = np.arange(start=1, stop=T5_re + 1, step=1) / float(T5_re)
x6_s = np.sort(b7_R_set_f)
T6_s = len(x6_s)
y6_s = np.arange(start=1, stop=T6_s + 1, step=1) / float(T6_s)
x6_re = np.sort(b7_R_reset_f)
T6_re = len(x6_re)
y6_re = np.arange(start=1, stop=T6_re + 1, step=1) / float(T6_re)
x7_s = np.sort(b8_R_set_f)
T7_s = len(x7_s)
y7_s = np.arange(start=1, stop=T7_s + 1, step=1) / float(T7_s)
x7_re = np.sort(b8_R_reset_f)
T7_re = len(x7_re)
y7_re = np.arange(start=1, stop=T7_re + 1, step=1) / float(T7_re)



###Plot CDF
plt.figure(3,figsize= (L/2.54,l/2.54))  ## CDF SET & RESET
...
if b5_R_set_f!=[]: plt.plot(x4_s, y4_s * 100, 'y.', markersize=dot_plot)
if b5_R_reset_f!=[]: plt.plot(x4_re, y4_re * 100, 'y*', markersize=dot_plot)
if b6_R_set_f!=[]: plt.plot(x5_s, y5_s * 100, 'c.', markersize=dot_plot)
if b6_R_reset_f!=[]: plt.plot(x5_re, y5_re * 100, 'c*', markersize=dot_plot)
if b7_R_set_f!=[]: plt.plot(x6_s, y6_s * 100, 'm.', markersize=dot_plot)
if b7_R_reset_f!=[]: plt.plot(x6_re, y6_re * 100, 'm*', markersize=dot_plot)
if b8_R_set_f!=[]: plt.plot(x7_s, y7_s * 100, 'k.', markersize=dot_plot)
if b8_R_reset_f!=[]: plt.plot(x7_re, y7_re * 100, 'k*', markersize=dot_plot)
# plt.ylim([0,3e-6])
#plt.xlim([0.8, 70])
plt.tick_params(axis='both',
                direction='in',
...
plt.subplots_adjust(left=0.15, right=0.99, top=0.99, bottom=0.2)
plt.xscale('log')
plt.xlabel("Résistance Log(k$\Omega$)",fontsize=font)
plt.ylabel("CDF (%)",fontsize=font)
#plt.grid()
plt.savefig(""+dos+"/CDF SET & RESET.svg")

plt.figure(4,figsize= (L/2.54,l/2.54))  ## CDF SET ONLY
...
plt.figure(5,figsize= (L/2.54,l/2.54))  ## CDF RESET ONLY
