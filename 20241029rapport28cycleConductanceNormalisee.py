# -*- coding: utf-8 -*-
"""
Created on Mardi October 30 15:37:32 2024
...
color = 'tab:red'
ax1.set_ylabel('C/Cp', fontsize=font, color=color)
ax1.plot(X0[:, 0], X0[:, 1], 'r.', X1[:, 0], X1[:, 1], 'k.', markersize=dot_plot)
ax1.plot(X1[:, 0], Cp[:], 'c-', label='Cp= ' + str(f"{(cp):.2f}") + ' uS')  # Plot Rp
ax1.plot(X1[:, 0], cth[:], 'b-', label='Cth=' + str(f"{(c_th/cp):.2f}") + ' Cp', markersize=dot_plot)  # Plot R th
ax1.tick_params(axis='y', labelcolor=color, labelsize=labsize)
ax1.set_yscale('log')
# ax1.set_ylim([1/100000, 1/100])
_a = 0
_b = 62  # Limits of x axis
ax1.set_xlim([_a,_b])
ax1.set_xticks([])
