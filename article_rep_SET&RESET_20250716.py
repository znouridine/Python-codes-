...
for g in range (min(len(a_s),len(b_s))):
    ...
    c_s = []
    e_s.append(b_s[g] - a_s[g])
    X2[0:abs(b_s[g]-a_s[g]), g+1]=X1[a_s[g]:b_s[g], 1]

X2[0:max(e_s), 0]=xx[0:max(e_s)]
...
for pop in range(_c,_d):
    if pop >= len(a_s):
        pop=len (a_s)-1
        plt.plot(X2[:,0], X2[:,pop+1], 'o-', markersize=dot_plot)
    plt.plot(X2[:,0], X2[:,pop+1], 'o-', markersize=dot_plot)
...
for g in range (min(len(a_re),len(b_re))):
    X3[0:abs(b_re[g]-a_re[g]), g+1]=X0[a_re[g]:b_re[g], 1]
    e_re.append(b_re[g] - a_re[g])
X3[0:max(e_re), 0]=xxx[0:max(e_re)]
...
for pop in range(_c,_d):
    if pop >= len(a_re):
        pop=len (a_re)-1
        plt.plot(X3[:,0], X3[:,pop+1], '1-', markersize=dot_plot)
    plt.plot(X3[:,0], X3[:,pop+1], '1-', markersize=dot_plot)
plt.tick_params(axis='y', labelcolor=color,labelsize=labsize)
plt.tick_params(axis='x', labelcolor=color,labelsize=labsize)
plt.yscale("log")
